...
export default function HistoricoPerguntas() {
    ...
    // do <input type="date"> nativo (que não abre de forma confiável dentro
    // do app instalado como PWA no iOS - reportado pelo usuário).
    ...
    useEffect(() => {
        ...
        fetch(`${API_URL}/controller/DailyQuestionController.php?action=historico`, {
            method: 'GET',
            headers: {
                ...
                <div className="space-y-3">
                    {historicoFiltrado.map((item, index) => (
                        ...
                        >
                            ...
                            {item.transcricao && (
                                <p className="text-gray-400 text-sm italic mb-2">"{item.transcricao}"</p>
                            )}

                            {item.feedback && (
                                <p className="text-gray-300 text-sm border-t border-gray-700 pt-2">{item.feedback}</p>
                            )}
                        </div>
                    ))}
                </div>
